...
studentNameInput.addEventListener("input", async ()=>{
  ...
  let out = "";
  d.matches.forEach(m=>{
    out += `<div class="autocompleteItem"
              data-id="${m.student_id}"
              data-name="${escapeHTML(m.student_name)}"
              data-grade="${escapeHTML(m.grade_level)}"
            >${escapeHTML(m.student_name)} (Grade ${escapeHTML(m.grade_level)})</div>`;
  });
  html(studentNameDropdown,out);
  show(studentNameDropdown);

  studentNameDropdown.querySelectorAll(".autocompleteItem").forEach(item=>{
    item.addEventListener("click", ()=>{
      studentNameInput.value = item.getAttribute("data-name");
      studentNameInput.dataset.studentId = item.getAttribute("data-id");
      hide(studentNameDropdown);
    });
  });
});
